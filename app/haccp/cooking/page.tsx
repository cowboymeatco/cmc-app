...
export default function CookingPage() {
  ...
  // Auto-refresh readings every 3 min when an active session is selected
  useEffect(() => {
    if (!selected || selected.status !== 'active') return
    const id = setInterval(() => loadReadings(selected.id), 3 * 60 * 1000)
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.dark, color: C.cream, fontFamily: 'system-ui, sans-serif', padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.75rem' }}>
        <Link href="/haccp" style={{ color: C.lightBrown, textDecoration: 'none', fontSize: '0.85rem' }}>
          ← HACCP
        </Link>

...
interface PriorityWeights {
  days_hanging:     number   // 0–10
  has_instructions: number   // 0–10 (penalty multiplier if missing)
  portion_size:     number   // 0–10
}
...
function calcScore(
  ...
  if (!entry.has_instructions) score -= w.has_instructions * 10  // big penalty for missing sheet
...
export default function CutSchedulePage() {
  ...
  const buildEntries = useCallback((
    ...
    for (const log of harvestLogs) {
      ...
      if (customers.length > 0) {
        // Expand one carcass into per-customer cut jobs
        for (const cust of customers) {
          ...
          raw.push({
            ...
            appointment_id:          log.id,           // stored as harvest_log_id in schedule table
        ...
        // No appointment customers — show carcass as a single standalone entry
        const saved = savedItems.find(
  ...
  // ── Load data from cooler inventory ──────────────────────────────────────────
  useEffect(() => {
    setLoading(true)
    Promise.all([
  ...
  // weights intentionally excluded — only recalculate on explicit button click
  ...
  const handleRecalculate = () => {
    setEntries(prev => {
      ...
      // Locked items keep their current position; unlocked re-sort by score
  ...
  // ── Render ────────────────────────────────────────────────────────────────────
  return (
    <div style={{ minHeight: '100vh', background: C.darkBrown, color: C.cream }}>
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <Link href="/" style={{ color: C.lightBrown, textDecoration: 'none', fontSize: '0.85rem' }}>
            ← Dashboard
          </Link>
          ...
          <div>
            <h1 style={{
              ...
              color: C.cream, letterSpacing: '0.06em', textTransform: 'uppercase', margin: 0,
            }}>
              🔪 Cut Schedule
            </h1>
        ...
        <button
          ...
          style={{
            background: saving ? C.medBrown : '#4CAF50',
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '1.5rem 1.5rem 6rem' }}>

        {/* Saved indicator */}
        {savedAt && (
          <div style={{
            background: 'rgba(76,175,80,0.12)', border: '1px solid rgba(76,175,80,0.3)',
            borderRadius: 4, padding: '0.5rem 1rem', marginBottom: '1rem',
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              color: C.tan, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600,
            }}
          >
            <span>⚖️ Priority Weights</span>
            <span style={{ fontSize: '0.75rem', color: C.lightBrown }}>
              {showWeights ? '▲ Hide' : '▼ Adjust'}
            </span>
          </button>
        ...
        {!loading && entries.length > 0 && (
          <div style={{
            ...
            {[
              { label: 'Animals', value: entries.length, color: C.tan },
              ...
              { label: 'Avg hang (days)', value: entries.length ? (entries.reduce((s, e) => s + e.days_hanging, 0) / entries.length).toFixed(1) : '0', color: C.lightBrown },
            ].map(stat => (
              <div key={stat.label} style={{
                ...
                <span style={{ fontSize: '1.15rem', fontWeight: 700, color: stat.color }}>{stat.value}</span>
                <span style={{ fontSize: '0.68rem', color: C.lightBrown, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{stat.label}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {!loading && entries.length === 0 && (
          <div style={{
            ...
            <p style={{ color: C.tan, fontSize: '1rem', margin: '0 0 0.5rem' }}>No animals in queue</p>
        ...
        {!loading && entries.length > 0 && (
          <>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: '28px 32px 1fr 80px 58px 76px 52px 48px 34px',
              gap: '0.5rem',
              padding: '0 0.75rem',
              marginBottom: '0.4rem',
            }}>
              {['', '#', 'Customer', 'Species', 'Cut', 'Hanging', 'Sheet', 'Score', ''].map((h, i) => (
                <span key={i} style={{ fontSize: '0.65rem', color: C.lightBrown, textTransform: 'uppercase', letterSpacing: '0.1em', textAlign: i >= 4 ? 'center' : 'left' }}>
                  {h}
                </span>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              {entries.map(entry => {
                ...
                const hangColor  = entry.days_hanging >= 10 ? C.red : entry.days_hanging >= 6 ? C.amber : '#4CAF50'
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'grab',
                      transition: 'border-color 0.1s, opacity 0.15s, box-shadow 0.1s',
                      boxShadow: isOver ? `0 0 0 2px ${C.amber}44` : 'none',
                    }}
                  >
                    {/* Drag handle */}
                    <span style={{ color: C.medBrown, fontSize: '1rem', userSelect: 'none', textAlign: 'center' }}>
                      ⠿
                    </span>

                    {/* Rank */}
                    <span style={{
                      fontFamily: 'Georgia, serif', fontSize: '1rem', fontWeight: 700, textAlign: 'center',
                      color: entry.rank <= 3 ? C.amber : C.lightBrown,
                    }}>
                      {entry.rank}
                    </span>

                    {/* Customer + carcass tag + notes */}
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', minWidth: 0 }}>
                        <span style={{
                          fontWeight: 600, fontSize: '0.88rem', color: C.cream,
                        ...
                        {entry.carcass_tag && (
                          <span style={{
                            fontSize: '0.68rem', color: C.medBrown, fontFamily: 'monospace',
                            background: 'rgba(0,0,0,0.3)', borderRadius: 2, padding: '0px 4px',
                            flexShrink: 0,
                          }}>
                            {entry.carcass_tag}
                          </span>
                        ...
                        {entry.hot_carcass_weight_lbs != null && (
                          <span style={{ fontSize: '0.68rem', color: C.lightBrown, flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      {entry.has_instructions ? (
                        <span style={{ color: '#4CAF50', fontSize: '1rem' }} title="Cut sheet on file">✓</span>
                      ) : (
                        <span style={{ color: C.red, fontSize: '0.78rem', fontWeight: 700 }} title="No cut sheet linked">⚠ Missing</span>
                      )}
                    </div>
            ...
            {/* Legend */}
            <div style={{
              marginTop: '1.5rem', padding: '0.65rem 1rem',
              background: 'rgba(0,0,0,0.25)', borderRadius: 4,
              display: 'flex', gap: '1.5rem', flexWrap: 'wrap',
              fontSize: '0.72rem', color: C.lightBrown,
            }}>
              <span>⠿ Drag to reorder</span>
              <span>🔒 Lock = pin to position when recalculating</span>
              <span style={{ color: C.red }}>⚠ Missing = no cut sheet linked</span>
              <span>
                Hanging: <span style={{ color: '#4CAF50' }}>0–5d</span>
